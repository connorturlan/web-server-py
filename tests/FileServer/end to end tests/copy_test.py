...
class FileServerCopyTests(unittest.TestCase):
	...
	def test_copy_file_fails_unspecifiedMethod(self):
	...
	def test_copy_file_fails_unspecifiedFile(self):
		response = send_patch('http://localhost/files/copy')

		self.assertEqual(response.status_code, 400)

	def test_copy_file_fails_missingBody(self):
		response = send_patch('http://localhost/files/copy/.test/.file.txt')

		self.assertEqual(response.status_code, 400)

	def test_copy_file_fails_invalidBody(self):
		response = send_patch('http://localhost/files',
		                      {"to": ".test_dest/file.txt"})

		self.assertEqual(response.status_code, 400)

	def test_copy_file_fails_destinationFolderDoesntExist(self):
		response = send_patch('http://localhost/files/copy/.test/.file.txt',
